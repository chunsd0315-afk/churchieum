    ...
    <div className="pb-8 space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 text-center">
          <CheckSquare className="w-6 h-6 text-success-500 mx-auto mb-1" />
          ...
          {WORSHIP_TYPES.map(w => {
            ...
            return (
              <button key={w.key} onClick={() => handleCheckIn(w.key)} disabled={checked}
                ...
                <span>{w.label}</span>
                {checked
                  ? <span className="flex items-center gap-1 text-success-600 text-xs"><Check className="w-4 h-4" /> 출석완료</span>
      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
        ...
        <div className="flex items-center gap-4 mt-3 text-xs text-gray-500">
          <span className="flex items-center gap-1"><div className="w-3 h-3 rounded-sm bg-success-500" /> 출석</span>
        ...
        <div className="divide-y divide-gray-50">
          {history.slice(0, 15).map(r => (
            <div key={r.id} className="flex items-center gap-3.5 px-4 py-3">
              <div className={`w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 ${r.is_present ? 'bg-success-100 text-success-600' : 'bg-red-100 text-red-400'}`}>
              ...
              <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${r.is_present ? 'bg-success-100 text-success-600' : 'bg-red-100 text-red-400'}`}>
                {r.is_present ? '출석' : '결석'}
              </span>
            </div>
          ))}
