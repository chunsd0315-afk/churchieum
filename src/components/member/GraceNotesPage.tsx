...
import {
  getAllGraceNotes, getGraceNotesByType, analyzeGraceNotes,
...
export default function GraceNotesPage() {
  ...
  const [editNote, setEditNote] = useState<GraceNote | null>(null);
  ...
  const readingNotes = getGraceNotesByType('reading');
  const sermonNotes  = getGraceNotesByType('sermon');
  ...
  const navToEdit = (note: GraceNote, from: SubView) => {
    ...
    if (note.type === 'reading') {
      const prog = activeProgresses.find(p => p.id === note.sourceId)
        ?? activeProgresses[0];
      setReadingCtx({
