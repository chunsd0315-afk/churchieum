...
import {
  ...
  Shield, Building, User, Phone, CheckCircle, ChevronRight,
} from 'lucide-react';
...
function PasswordStrength({ pw }: { pw: string }) {
  ...
  return (
    <div className="mt-2 space-y-1">
      ...
      <p className={`text-xs ${strength<=1?'text-red-500':strength===2?'text-amber-500':'text-secondary-600'}`}>{labels[strength]}</p>
    </div>
  );
}
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input type={show2?'text':'password'} value={pw2} onChange={e=>{setPw2(e.target.value);setError('');}}
                ...
                className={`w-full pl-11 pr-12 py-3.5 bg-white border rounded-xl text-sm focus:ring-0 ${pw2&&pw2===pw?'border-secondary-400 focus:border-secondary-400':'border-gray-200 focus:border-primary-400'}`} />
