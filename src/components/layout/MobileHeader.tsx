...
export function MobileHeader({
  ...
  const profileRole = mode === 'admin' ? 'super_admin' as const : mode === 'pastor' ? 'pastor' as const : 'member' as const;
  ...
  return (
    <header className="sticky top-0 z-50 h-16 bg-white border-b border-gray-200 flex items-center px-4 gap-3 shrink-0">
      ...
      >
        <UserProfileAvatar
          user={user ? { id: user.id, name: user.name, role: profileRole } : null}
          size={36}
        />
