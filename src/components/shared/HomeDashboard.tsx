...
const MENUS: MenuCard[] = [
  ...
  { id: 'qt',            label: '은혜기록',    icon: BookHeart,      bg: 'bg-primary-50', ic: 'text-primary-500', roles: ['admin', 'pastor', 'member'] },
      ...
      {menus.map(item => (
        ...
        >
          <div className={`w-12 h-12 ${item.bg} rounded-[14px] flex items-center justify-center group-hover:scale-110 transition-transform duration-150`}>
            <item.icon className={`w-5.5 h-5.5 ${item.ic}`} style={{ width: 22, height: 22 }} />
          </div>
